...
import { InContextTab } from './tabs/InContextTab';

const TAB_KEYS = ['preview', 'docs', 'examples', 'in-context', 'code', 'styles'] as const;
...
const TABS: { key: TabKey; label: string; icon: string }[] = [
  ...
  { key: 'in-context', label: 'In Context', icon: 'integration_instructions' },
    ...
    <Box>
      ...
      <Box sx={{ mb: 3 }}>
        <Box sx={{ display: 'flex', alignItems: 'center', gap: 1.5, mb: 1 }}>
          ...
          <Chip
            label={doc.name.includes('[Base]') ? 'Base' : 'MUI'}
            size="small"
            variant="outlined"
            color={doc.name.includes('[Base]') ? 'secondary' : 'default'}
      ...
      <Box>
        ...
        {activeTab === 'in-context' && <InContextTab doc={doc} />}
